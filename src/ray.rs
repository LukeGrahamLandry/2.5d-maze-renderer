...
use crate::light_cache::{LightCache, LightingRegion, PortalLight};
...
pub(crate) trait SolidWall<'map, 'walls> {
...
#[derive(Clone, Copy)]
pub(crate) struct Portal<'map, 'walls> {
    pub(crate) from_wall: &'walls (dyn SolidWall<'map, 'walls> + Sync),
    pub(crate) to_wall: &'walls (dyn SolidWall<'map, 'walls> + Sync)
}

impl<'map: 'walls, 'walls> Portal<'map, 'walls> {
...
impl <'map: 'walls, 'walls> LightCache<'map, 'walls> {
    ...
    pub(crate) fn ray_trace(&'walls self, start_region: &'walls LightingRegion<'map, 'walls>, mut origin: Position, mut direction: Direction) -> Vec<RaySegment<'map, 'walls>> {
        let mut segments = vec![];

        let mut segment: RaySegment<'map, 'walls> = start_region.single_ray_trace(origin, direction);
                ...
                Some(hit_wall) => {
                    match hit_wall.portal() {
                        ...
                        Some(portal) => {
                            ...
                            let next_region = self.get_lighting_region(portal.to_wall.region());
    ...
    pub(crate) fn trace_clear_path_with_portals_between(&'walls self, start_region: &'walls LightingRegion<'map, 'walls>, origin: Vector2, target: Vector2) -> Option<Vec<RaySegment<'map, 'walls>>> {
...
impl<'map: 'walls, 'walls> LightingRegion<'map, 'walls> {
    ...
    pub(crate) fn trace_clear_path_no_portals_between(&'walls self, origin: Vector2, target: Vector2) -> Option<RaySegment<'map, 'walls>> {
        let direction = target.subtract(&origin).normalize();
...
//       but also make sure it doesn't devolve into new RayCasterManagerAbstractFactoryBuilder().build().run(...)
    ...
    pub(crate) fn trace_clear_portal_light(&'walls self, light: &'walls PortalLight<'map, 'walls>, target: Vector2) -> Option<LineSegment2> {
    ...
    pub(crate) fn single_ray_trace(self: &'walls LightingRegion<'map, 'walls>, origin: Vector2, direction: Vector2) -> RaySegment<'map, 'walls> {
    ...
    pub(crate) fn find_shortest_path(&'walls self, pos: Vector2, wall_normal: Vector2, wall: LineSegment2) -> Option<RaySegment<'map, 'walls>> {
        let sample_count = (wall.length() / LightingRegion::PORTAL_SAMPLE_LENGTH).floor();
...
#[derive(Clone)]
pub struct RaySegment<'map, 'walls> {
    pub(crate) region: &'walls LightingRegion<'map, 'walls>,
    pub(crate) line: LineSegment2,
    pub(crate) hit_wall: Option<&'walls dyn SolidWall<'map, 'walls>>
}

impl<'map, 'walls> RaySegment<'map, 'walls> {
    pub(crate) fn empty(region: &'walls LightingRegion<'map, 'walls>, origin: Vector2, direction: Vector2) -> RaySegment<'map, 'walls> {
        RaySegment {
            region,
    ...
    pub(crate) fn hit(region: &'walls LightingRegion<'map, 'walls>, wall: &'walls (dyn SolidWall<'map, 'walls>), line: LineSegment2) -> RaySegment<'map, 'walls> {
        RaySegment {
            region,
            line,
            hit_wall: Some(wall)
